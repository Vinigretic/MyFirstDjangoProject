...
# def products(request, product_id = 0): # передаем в функцию доп аргумент со значением по умолчанию, можно передавать
                                         # так же без значения
...
# urlpatterns = [
#     path('products/', views.products),  # описываем путь к views, с аргументом по умолчанию, это делаем всегда когда
#                                         # у нас есть такой аргумент
#     path('products/<int:product_id>/', views.products), # описываем путь к vie0ws, с доп аргументом
...
# str: соответствует любой строке за исключенем символа "/". Если спецификатор не указан, то используется
# по умолчанию
